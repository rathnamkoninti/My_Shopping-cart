...
from app.models import Product, Cart, OrderPlaced
from app.forms import CustomerRegistrationForm,CustomerProfileForm, Customer
...
def show_cart(request):
    ...
    if request.user.is_authenticated:
        ...
        if cart_product:
            ...
            return render(request, 'app/addtocart.html',{'carts':cart, 'totalamount':totalamount, 'amount':amount,'cart_product':cart_product,'totalitem':totalitem})
        else:
...
def password_reset(request):
 return render(request, 'app/password_resetss.html')
...
def laptop(request, data=None):
    if data==None:
        laptop = Product.objects.filter(category='L')
    elif data=='Dell' or data=='Lenova' or data=='Hp':
...
class ProfileView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            reg.save()
            messages.success(request, 'Congratulations!!! Profile Updated successfullyy')
        return render(request,'app/profile.html',{'form':form, 'active':'btn-primary'})
